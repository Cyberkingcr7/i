...
browser: ['Bii moders','Safari','1.0.0'],
auth: state,
})
...
//=================================================//
    ...
    // Get Profile Picture Group
       try {
       ppgc = await haikal.profilePictureUrl(ciko.id, 'image')
       } catch {
       ppgc = 'https://tinyurl.com/yx93l6da'
       }
       let wm_fatih = { url : ppgc }
       if (ciko.announce == true) {
       haikal.send5ButImg(ciko.id, `「 Group Settings Change 」\n\nGroup telah ditutup oleh admin, Sekarang hanya admin yang dapat mengirim pesan !`, `Group Settings Change Message`, wm_fatih, [])
    ...
    haikal.ev.on('group-participants.update', async (anu) => {
        ...
        try {
            ...
            for (let num of participants) {
                ...
                if (anu.action == 'add') {
                    haikal.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `───「 *SELAMAT DATANG* 」────
 ☎️ Nomor : @${num.split("@")[0]}
 🔰 Group : ${metadata.subject}
 📝 *DESC* : ${metadata.desc}` })
                } else if (anu.action == 'remove') {
                    haikal.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: ` ───「 *Selamat Tinggal Di Group* 」────
 ☎️ Nomor : @${num.split("@")[0]}
 🔰 Group : ${metadata.subject}
 📝 *DESC* : ${metadata.desc}` })
